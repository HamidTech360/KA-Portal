...
const Profile = () => {
    ...
    const handleEdit = async (field)=>{
        ...
        try{
            const response = await axios.put(`${config.apiUrl}/api/admins`, {[field]:formValues[field]}, {headers:{
    ...
    const {isLoading, data:profile} = useQuery(
        ...
        ()=>getRequest({url:`api/admins/${profileId}`}),
        {
            onSuccess(response){
    ...
    const profileData  = profile?.data?.admin
    useEffect(()=>{
        ...
        <HomeLayout>
            <div className={styles.pageContainer}>
                ...
                <div className={`${Pstyles.profileDisplay} d-flex`}>
                    <div>
                        <Avatar name={profileData?.userName} round={true} size={window.innerWidth > 768?'5rem':'4rem'}/>
                    </div>
